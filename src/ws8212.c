...
#define ONECOLOR 8
#define ONEPIXEL 3 * ONECOLOR
#define BUFSIZE (NUMLED * ONEPIXEL)

uint32_t realbuf[BUFSIZE + 64] = {0};
uint32_t *buf = realbuf + 32;
...
void ws8212_update(void) {
  HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_3, realbuf, BUFSIZE + 64);
}

void setcolor(int index, uint8_t color) {
    for(int i = 0; i < ONECOLOR; i++) {
        buf[index + 7 - i] = LOW + (!!(color & (1 << i))) * (HIGH - LOW);
    }
}

void ws8212_set(int index, uint8_t r, uint8_t g, uint8_t b) {
    int base = index * ONEPIXEL;
    setcolor(base + 0 * ONECOLOR, g);
    setcolor(base + 1 * ONECOLOR, r);
    setcolor(base + 2 * ONECOLOR, b);
